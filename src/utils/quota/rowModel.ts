 ...
 * scanned top to bottom. That only works if the five provider states — which
 ...
 */
export type QuotaProviderFamily = 'antigravity' | 'claude' | 'codex' | 'kimi' | 'xai';
...
export interface QuotaCredentialRowModel {
  ...
  renewal: { label: string; atMs: number | null } | null;
  notes: QuotaRowNote[];
