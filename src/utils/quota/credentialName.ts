...
export function maskCredentialName(name: string): string {
  ...
  const identityIndex = name.indexOf(identitySeparator);
  if (identityIndex >= 0) {
    const prefix = name.slice(0, identityIndex + identitySeparator.length);
    const identity = name.slice(identityIndex + identitySeparator.length);
    return `${prefix}${maskCredentialName(identity)}`;
  }
