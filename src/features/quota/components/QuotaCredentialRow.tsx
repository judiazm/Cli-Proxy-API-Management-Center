 ...
 *
 * Behaviour carried over from the card unchanged: click-to-load (quota is
 * never fetched on mount), per-credential refresh, and the Codex manual reset
 * with its confirmation flow.
 */
...
export function QuotaCredentialRow(props: QuotaCredentialRowProps) {
  ...
  const displayName = displayCredentialName(file.name, showEmails);
            ...
            <div className={styles.sub}>
              ...
              {model?.renewal && (
                <span className={styles.renewal}>
                  <span className={styles.renewalLabel}>{t('codex_quota.expires_label')}</span>
                  <QuotaCountdown
